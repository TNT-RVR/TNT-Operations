...
import type { ArchivedUser, CalendarFeed, CompanyDetails, DocumentSignature, GcalStatus, QboStatus, UserSignature } from './types'
...
export interface SettingsSlice {
  ...
  /**
   * The subscribable calendar feed. Null before migration 0023.
   */
...
export function useSettings(currentUserId: string | null, live: boolean): SettingsSlice {
  ...
  const [calendarFeed, setFeed] = useState<CalendarFeed | null>(null)
  ...
  const loadSettings = useCallback(async (): Promise<void> => {
    ...
    const [co, ov, sig, docSigs, qbo, people, gcal, feed] = await Promise.all([
      ...
      sb.from('calendar_feed').select('*').limit(1).maybeSingle(),
    ])
    ...
    if (feed.error) console.warn('[settings] calendar feed:', feed.error.message)
    else if (feed.data) {
  ...
  const regenerateFeedToken = useCallback(async (): Promise<SettingsResult> => {
    if (!sbase) return { ok: false, error: 'Not connected' }
    // Server-side RPC: the new token is generated by pgcrypto, never by a
  ...
  const setFeedEnabled = useCallback(
    async (enabled: boolean): Promise<SettingsResult> => {
      if (!sbase) return { ok: false, error: 'Not connected' }
      const { error } = await sbase.from('calendar_feed').update({ enabled }).eq('id', true)
      if (error) return { ok: false, error: error.message }
      setFeed((f) => (f ? { ...f, enabled } : f))
      return { ok: true }
    },
    [sbase],
  )
