...
--
-- NAMED `app_notifications` (not `notifications`) so it coexists with the old
-- beetent-maps app's differently-shaped `public.notifications` table in this
...
create index if not exists app_notifications_active_idx
  on public.app_notifications (created_at desc) where deleted_at is null;
-- One active (undeleted, unread) alert per dedup_key — a stalled feed shouldn't
-- pile up a new alert every cycle.
create unique index if not exists app_notifications_dedup_idx
  on public.app_notifications (dedup_key)
...
create table if not exists public.app_notification_prefs (
...
alter table public.app_notification_prefs enable row level security;
...
-- bypasses RLS).
drop policy if exists "notif read" on public.app_notifications;
create policy "notif read" on public.app_notifications
  for select to authenticated using (public.has_access());
drop policy if exists "notif update" on public.app_notifications;
create policy "notif update" on public.app_notifications
  for update to authenticated using (public.has_access()) with check (public.has_access());
drop policy if exists "notif delete" on public.app_notifications;
create policy "notif delete" on public.app_notifications
  for delete to authenticated using (public.has_access());
drop policy if exists "notif insert" on public.app_notifications;
create policy "notif insert" on public.app_notifications
  for insert to authenticated with check (public.can_edit());

-- Prefs: each user manages only their own rows.
drop policy if exists "prefs own" on public.app_notification_prefs;
create policy "prefs own" on public.app_notification_prefs
